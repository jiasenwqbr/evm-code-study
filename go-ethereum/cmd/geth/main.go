...
/*
这就是 geth 命令行客户端 的入口程序。它做三件事：
搭建 CLI（命令行）框架：定义所有全局 flag、子命令（init / import / console / db ...），把它们注册到 urfave/cli。
默认命令逻辑：当不带子命令时（即直接跑 geth），创建并启动一个“完整节点”，开启 RPC/IPC、载入钱包、监听同步状态等。
运行前/后的准备与清理：设置 GOMAXPROCS、调试系统、环境变量映射、退出时清理终端模式等。
*/
