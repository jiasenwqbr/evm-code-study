...
// Block 代表一个以太坊区块。
//
// 请注意，Block 类型力求“不可变”，并包含依赖于此的某些缓存。关于区块不可变性的规则如下：
//
// - 我们在构造区块时复制所有数据。这使得区块内部的引用与传入的值无关。
//
// - 我们在访问时复制所有区块头数据。这是因为对区块头的任何更改都会扰乱区块中缓存的哈希值和大小值。调用代码应该利用这一点来避免过度分配！
//
// - 当新的区块主体数据附加到区块时，将返回该区块的浅拷贝。
// 这确保了区块修改不会发生竞争。
//
// - 我们在访问时不复制区块主体数据，因为这不会影响缓存，而且
// 这样做成本太高。
type Block struct {
	header       *Header
	uncles       []*Header
	...
	// that process it.
	// witness （见证信息）不是区块主体的编码部分。
	// 它被保存在区块中，以便于轻松传递到处理它的地方。
